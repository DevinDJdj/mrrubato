...
if __name__ == '__main__':
    ...
    if (len(qas) > 0):
        fg = FeedGenerator()
        y = args.d[0:4]
        fg.id('http://misterrubato.com/rss/' + y + '/' + args.d)
        fg.title(args.d)
        ...
        for qa in qas:
            qnum = qnum + 1
            fe = fg.add_entry()
            fe.id('http://misterrubato.com/rss/' + y + '/' + args.d + '/' + str(qnum))
            print(qa)
            fe.title(qa["question"])
            fe.author({"name": qa["author"], "uri": "https://www.youtube.com/channel/" + qa["authorchannel"]})
            fe.content(parseAnswer(qa["answer"]))

    atomfeed = fg.atom_str(pretty=True) # Get the ATOM feed as string
    rssfeed  = fg.rss_str(pretty=True) # Get the RSS feed as string
    pathlib.Path('web/public/rss/' + y + '/' + args.d).mkdir(parents=True, exist_ok=True) 
    fg.atom_file('web/public/rss/' + y + '/' + args.d + '/atom.xml') # Write the ATOM feed to a file
    fg.rss_file('web/public/rss/' + y + '/' + args.d + '/rss.xml') # Write the RSS feed to a file
