...
combobox bounds(  5, 270, 90, 20), channel("source"), value(1), items("Sine","VCO","Buzz","FM/RM/AM","Hsboscil","PD Half X","PD Half Y","FOF","Bit Depth","S.R. Rate","pdclip","powershape","clip","hilbert","distort1","grain","polynomial","chebyshevpoly","alpass")
...
combobox   bounds(  5, 38,100, 25), channel("displaytype"), items("Spectrogram","Waveform")
...
combobox   bounds(175, 38, 80, 25), channel("wsize"), items("256","512","1024","2048","4096","8192","16384","32768"), value(3)
rslider    bounds(260, 20, 60, 60), channel("minbin"), text("Min.Bin"), range(0,511,0,1,1)
label      bounds(325, 25, 80, 12), text("Max.Bin")
combobox   bounds(325, 38, 80, 25), channel("maxbin"), items("Win/2","Win/4","Win/8","Win/16","Win/32","Win/64"), value(4)
...
signaldisplay bounds(  5,90,500,300), colour("cornflowerblue"), identchannel("fft"), displaytype("spectrogram"), signalvariable("asig","ascan")
}
...
instr 1000				; launches plant popups
 ...
 if changed:k(kdisplaytype)==1 then
	if kdisplaytype==0 then
	 chnset "displaytype(\"spectroscope\")", "fft"
	elseif kdisplaytype==1 then
	 chnset "displaytype(\"spectrogram\")", "fft"
	elseif kdisplaytype==2 then
	 chnset "displaytype(\"waveform\")", "fft"
	elseif kdisplaytype==3 then
	 chnset "displaytype(\"lissagous\")", "fft"
	endif
 endif
