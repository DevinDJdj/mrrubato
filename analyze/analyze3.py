#analyze all midi data for one song.  
#see the patterns within this.  
...
#                        os.spawn("python", cmd, no_wait=True)
            ...
            if (len(starttimes) != len(endtimes)):
                ...
                for idx,t in enumerate(starttimes):
                    totalidx +=1
                    mydata = {"id": videoid, "midilink": midilink, "transcriptlink": transcriptlink, "title": title, "GroupName": GroupName, "URL": url, "PublishedDate": publishedDate, "starttime": starttimes[idx], "endtime": endtimes[idx], "iteration": totalidx}
#                        print(mydata)
                    iterations.append(mydata)


    #sort iterations.  
        iterations = sorted(iterations, key=lambda x: (x['PublishedDate'], x['starttime']))    
    
    #get midi for each iteration.  
    cnt = 0
    ...
    for i in iterations:
        ...
        if (videoid is not None):
            if (videoid == latestvideo):
                ...
                webbrowser.open('https://www.misterrubato.com/analyze.html?video=' + videoid, autoraise=False)

            item = i
            #some other function needed.  
            printMidi(item['midilink'], item['title'], item['GroupName'], item['id'])
            printTranscript(item['transcriptlink'])            
