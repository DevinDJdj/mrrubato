...
exports.getTopicFromFilePath = getTopicFromFilePath;
...
exports.loadGenBook = loadGenBook;
...
exports.getGenBookPath = getGenBookPath;
...
exports.ollama_model = 'gemma3:4b';
...
function getMyUris(topic, externalpath = "") {
    ...
    if (externalpath !== "") {
        ...
        folderUri = folderUri.with({ path: path_1.posix.join(folderUri.path, externalpath) });
    }
    ...
    if (fs.existsSync(fileUri.fsPath)) {
        if (fs.lstatSync(fileUri.fsPath).isDirectory()) {
            ...
            for (const file of files) {
                ...
                const tempUri = folderUri.with({ path: path_1.posix.join(folderUri.path, topic, file) });
                ret.push(tempUri);
            }
...
function pickTopic(selectedtopics, defaultprompts = [], numtopics = 10, extendtopics = false, numentries = 10) {
    ...
    if (extendtopics || selectedtopics.length === 0) {
        if (exports.selectionhistory.length > 0) {
            for (let i = exports.selectionhistory.length - 1; i > -1 && i > exports.selectionhistory.length - numtopics; i--) {
...
function getTopicFromFilePath(filePath, wsf = vscode.workspace.workspaceFolders?.[0]) {
...
function updatePage(filePath, text, linefrom = 0, lineto = 0, show = false) {
    ...
    try {
        ...
        fs.appendFileSync(path_1.posix.join(transcriptFolder, 'genbook\\' + copyUri), text + "\n", 'utf-8'); //write the text to the transcript folder for backup and analysis.
    }
    catch (error) {
    ...
    if (filePath.startsWith(genbookfolder)) {
        ...
        try {
            console.log('writing to ' + path_1.posix.join(folderUri.path, genbookfolder + "\\" + copyUri));
            fs.appendFileSync(path_1.posix.join(folderUri.fsPath, genbookfolder + "\\" + copyUri), text + "\n", 'utf-8');
        }
        catch (error) {
            console.error(`Error writing file: ${error}`);
        }
...
function loadGenBook(context = null) {
